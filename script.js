...
// Physics stability: keep simulation at a fixed 60 Hz even when display/rAF drops
// to 30 Hz in Low Power Mode. This preserves the original feel while avoiding
// oversized variable-dt steps that can excite rope/charm jitter.
...
function simulatePhysicsStep() {
  ...
  for (const state of states) {
    if (!state.dragging) {
      state.vy += CHARM_GRAVITY;
      applyTautRopeForce(state, dt);
      state.vx *= CHARM_DAMPING;
      state.vy *= CHARM_DAMPING;
      capVelocity(state);
      state.x += state.vx;
      state.y += state.vy;
  ...
  for (const state of states) {
    ...
    if (!state.dragging) {
      ...
      state.rotation += (desiredRotation - state.rotation) * .105;
    } else {
      state.rotation += (clamp(state.vx * 1.16, -22, 22) - state.rotation) * .19;
    }
    state.collisionPulse *= .74;
  }
}
...
function tick(now) {
  // Variable display frame pacing (especially iOS Low Power Mode) no longer changes
  // the size of a physics step. A ~33 ms display frame simply runs two 60 Hz steps.
  const frameMs = clamp(now - lastFrame, 0, MAX_FRAME_GAP_MS);
  lastFrame = now;
  physicsAccumulator += frameMs;

  let steps = 0;
  while (physicsAccumulator >= FIXED_STEP_MS && steps < MAX_STEPS_PER_FRAME) {
    simulatePhysicsStep();
    physicsAccumulator -= FIXED_STEP_MS;
    steps += 1;
  }

  // If a tab/background stall was longer than we are willing to catch up, discard
  // excess accumulated time instead of injecting a burst of energy on resume.
  if (steps === MAX_STEPS_PER_FRAME && physicsAccumulator >= FIXED_STEP_MS) {
    physicsAccumulator %= FIXED_STEP_MS;
  }
...
window.__summerflowPhysics = {
  version: '02-2B-low-power-fixed-step-v10',
